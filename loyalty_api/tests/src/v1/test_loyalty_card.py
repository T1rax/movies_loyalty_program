...
async def test_create_card(test_api_client, test_app):
    with test_app.container.loyalty_repository.override(
        ...
        response = test_api_client.post("api/v1/loyalty_cards")

    # Такой пользователь уже существует, поэтому тут ожидаем ошибку
    # Корректный сценарий проверяется в srv
...
async def test_change_loyalty_level(
    ...
    with test_app.container.loyalty_repository.override(
        ...
        response = test_api_client.post(
            "api/v1/loyalty_cards/change_level",
            json={"action": action, "loyalty_level": level},
        )
...
async def test_get_card_info(test_api_client, test_app):
    with test_app.container.loyalty_repository.override(
        ...
        response = test_api_client.get("api/v1/loyalty_cards")
...
async def test_get_card_balance_history(test_api_client, test_app):
    with test_app.container.loyalty_repository.override(
        ...
        response = test_api_client.get("api/v1/loyalty_cards/history")
...
async def test_loyalty_card_refill(test_api_client, test_app):
    with test_app.container.loyalty_repository.override(
        ...
        response = test_api_client.post(
            "api/v1/loyalty_cards/refill",
            json={"points": 100, "source": "test"},
        )
